...
## Process CSS
CUR_DIR=`pwd`
for d in `find $CSS_DIR -type d -mindepth 1 | grep -v $EXCLUDED_DIR`;
do
	cd $CUR_DIR"/"$d;
	rm all.css
...
## Process JS
CUR_DIR=`pwd`
for d in `find $JS_DIR -type d -mindepth 1 | grep -v $EXCLUDED_DIR`;
do
        cd $CUR_DIR"/"$d;
	rm all.js
